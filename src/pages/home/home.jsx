    ...
    <main className="min-h-screen">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-slate-50 via-white to-slate-100">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-gold/5 rounded-full blur-3xl animate-float" style={{ animationDelay: "1s" }} />
        </div>

        <div ref={heroRef} className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 text-center opacity-0">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/80 backdrop-blur-sm border border-slate-200/50 mb-8 animate-fade-in-down">
            <HiSparkles className="w-4 h-4 text-gold" />
            <span className="text-xs font-semibold text-slate-700 uppercase tracking-wider">Premium Audio Solutions</span>
          </div>

          <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-bold mb-6 animate-fade-in-up" style={{ animationDelay: "0.1s" }}>
            <span className="block text-slate-900">Sound that</span>
            <span className="block gradient-text mt-2">moves the room</span>
          </h1>

          <p className="text-lg sm:text-xl text-slate-600 max-w-2xl mx-auto mb-12 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
            Professional audio equipment and lighting for events, studios, and venues. Rent or buy — we deliver excellence.
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-4 animate-fade-in-up" style={{ animationDelay: "0.3s" }}>
            ...
            <Link
              to="/contact"
              className="inline-flex items-center px-8 py-4 rounded-2xl border-2 border-slate-300 text-slate-700 font-semibold hover:border-accent hover:text-accent hover:bg-accent/5 transition-all duration-300"
            >
              Get in Touch
            </Link>
          </div>
      ...
      {/* Features Section */}
      <section ref={featuresRef} className="py-24 md:py-32 bg-white opacity-0">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-bold text-slate-900 mb-4">Why Wave Audio?</h2>
            <p className="text-lg text-slate-600 max-w-2xl mx-auto">Excellence in every detail</p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              ...
              <div
                key={item.title}
                className="group p-8 rounded-3xl bg-gradient-to-br from-white to-slate-50 border border-slate-200/80 shadow-card hover:shadow-premium transition-all duration-500 hover:-translate-y-2"
