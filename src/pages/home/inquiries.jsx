...
import { HiMail, HiPhone, HiCalendar, HiCheckCircle, HiXCircle, HiPencil, HiTrash } from "react-icons/hi";
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-white to-slate-50 py-12 md:py-20">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {showForm && (
          <div className="rounded-3xl bg-white border border-slate-200 shadow-card p-6 md:p-8 mb-8 animate-fade-in">
            <h2 className="text-2xl font-bold text-slate-900 mb-6">Create New Inquiry</h2>
        ...
        {inquiries.length === 0 ? (
          <div className="text-center py-20 rounded-3xl bg-white border border-slate-200 shadow-card">
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-2">No inquiries yet</h2>
            <p className="text-slate-600">
              {isAdmin ? "No customer inquiries at the moment" : "Create your first inquiry"}
            </p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-xl font-bold text-slate-900">Inquiry #{inquiry.id}</h3>
                      {inquiry.isResolved ? (
                ...
                {editingId === inquiry.id ? (
                  <div className="space-y-3">
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-4 py-2 rounded-xl border-2 border-accent focus:ring-2 focus:ring-accent/20 outline-none"
                      ref={(el) => {
                        if (el) el.focus();
                      }}
                      onBlur={(e) => {
                  ...
                  <p className="text-slate-700 mb-4 leading-relaxed">{inquiry.message}</p>
                )}

                {inquiry.response && (
                  <div className="mt-4 p-4 rounded-xl bg-accent/5 border border-accent/20">
                    <p className="text-sm font-semibold text-accent mb-1">Response:</p>
                    <p className="text-slate-700">{inquiry.response}</p>
                  </div>
                )}
